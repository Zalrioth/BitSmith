...
namespace BitSmith
{
	class Water
	{
		...
		RenderTarget2D metaballTarget, particlesTarget, drawTarget;
		...
		public void DrawToRenderTargets()
		{
            ...
            //spriteBatch.Begin(0, BlendState.Additive, null, null, null, null, Game1.cam.get_transformation(Game1.graphics.GraphicsDevice));
            spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.Additive, SamplerState.PointClamp, null, null, null, Game1.cam.get_transformation(Game1.graphics.GraphicsDevice));
			/*foreach (var particle in particles)
			{
            ...
            for (int i = 0; i < Game1._fluidSimulation.getActiveParticles(); i++)
            {
                //Particle particle = _liquid[_activeParticles[i]];
                //Particle particle = Game1._fluidSimulation.getLiquidList()[Game1._fluidSimulation.getActiveParticlesList()[i]];
                FluidParticle particle = Game1._fluidSimulation.getParticle(i);
                //FluidParticle particle = FluidSimulation._liquid[FluidSimulation._activeParticles[i]];
                ...
                //spriteBatch.Draw(_pixel, (particle.position) * Game1.scale, new Rectangle(0, 0, 2, 2), Color.DarkBlue, 0f, new Vector2(1, 1), 1f, SpriteEffects.None, 0f);
                Vector2 origin = new Vector2(particleTexture.Width, particleTexture.Height) / 2f;
                //spriteBatch.Draw(particleTexture, particle.position * Game1.scale, null, Color.White, 0, origin, 2f, 0, 0);
                //Color lightBlue = new Color(0.2f, 0.5f, 1f, 0.45f);
                //spriteBatch.Draw(particleTexture, particle.position * Game1.scale, null, new Color(1f, 1f, 1f, 1f), 0, origin, 2f, 0, 0);
                spriteBatch.Draw(particleTexture, particle.position * Game1.scale, null, new Color(1f,1f,1f,1f), 0, origin, 2f, 0, 0);
            }
			spriteBatch.End();
            ...
            //spriteBatch.Draw(metaballTarget, Vector2.Zero, new Color(1f,1f,1f,1f));
            spriteBatch.Draw(metaballTarget, Vector2.Zero, Color.Transparent);
			spriteBatch.End();
			
            device.SetRenderTarget(drawTarget);
            device.Clear(Color.Transparent);
            Color lightBlue = new Color(0.2f, 0.5f, 1f, 0.5f);

            spriteBatch.Begin();
            spriteBatch.Draw(particlesTarget, -Vector2.One, new Color(0.8f, 0.8f, 1f, 0.5f));
        ...
        {
            spriteBatch.Begin();
            spriteBatch.Draw(drawTarget, new Vector2(0, 0), _color);
            spriteBatch.End();
        }
	}
}
