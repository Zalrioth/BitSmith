...
namespace BitSmith
{
    ...
    {
        public const bool DRAW_CELLS = true;
        ...
        private bool clearParticles = false;
        private int _numActiveParticles = 0;
        private FluidParticle[] _liquid;
        private FluidParticle[] _liquidDelete;
        ...
        {
            FluidParticle particle = _liquid[index];
                    ...
                    for (int i = Ax; i < Bx; i++)
                    {
                        for (int j = Ay; j < By; j++)
                        {
                            if (_grid.TryGetValue(i, out collisionGridX) && collisionGridX.TryGetValue(j, out collisionGridY))
                            {
                                ...
                                {
                                    FluidParticle particle = _liquid[collisionGridY[k]];
                                    if (particle.numFixturesToTest < FluidParticle.MAX_FIXTURES_TO_TEST)
                                    {
        ...
        {
            IEnumerable<FluidParticle> inactiveParticles = from particle in _liquid
                                                      where particle.alive == false
                                                      select particle;
            inactiveParticles = inactiveParticles.Take(numParticlesToSpawn);

            foreach (FluidParticle particle in inactiveParticles)
            {
        ...
        public void draw()
        {
            _spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.NonPremultiplied, SamplerState.PointClamp, null, null, null, Game1.cam.get_transformation(Game1.graphics.GraphicsDevice));
            //_spriteBatch.Begin();
            // Draw cells
            if (DRAW_CELLS)
            {
                ...
                foreach (KeyValuePair<int, Dictionary<int, List<int>>> row1 in _grid)
                {
                    foreach (KeyValuePair<int, List<int>> row2 in row1.Value)
                    {
                        _spriteBatch.Draw(_pixel, (new Vector2((float)row1.Key * CELL_SIZE, (float)row2.Key * CELL_SIZE) * Game1.scale), new Rectangle(0, 0, gridSize, gridSize), Color.DarkBlue);
                        //Game1.checkGrid(new Vector2((float)row1.Key * CELL_SIZE, (float)row2.Key * CELL_SIZE));
                    }
                }
            }
            // Draw particles
            ...
            if (_liquid.Count() > 0)
                _spriteBatch.DrawString(_font, "" + _liquid[0].position, new Vector2(500, 8), Color.White);
        ...
        public FluidParticle[] getLiquidList()
        {
            return _liquid;
        }
        public List<int> getActiveParticlesList()
        {
            return _activeParticles;
        }
        public FluidParticle getParticle(int _i)
        {
            return _liquid[_activeParticles[_i]];
        }
        public void emptyParticles()
        {
            clearParticles = true;
        }
    }
